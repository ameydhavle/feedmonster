...
define variable $sub-rp as xs:string external

xdmp:node-replace(
    fn:collection(feed:subscription())/feed:subscription[@id = $sub-id]/feed:reference-preferred,
    <reference-preferred xmlns="http://marklogic.com/xdmp/feed">{$sub-rp}</reference-preferred>
)