...
define variable $sub-lr as xs:dateTime external

        xdmp:node-replace(
            fn:collection(feed:subscription())/feed:subscription[@id = $sub-id]/@last-requested,
            attribute last-requested { $sub-lr }
        )