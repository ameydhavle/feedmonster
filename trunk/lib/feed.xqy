...
define variable $COL-HTML {"/html/doc"}
...
as empty() {
    ...
    let $sub-doc := 
     ...
     let $t2 :=
         ...
         then    xdmp:spawn(
                 '/feed/lib/actions/subscription-request.xqy',
...
(:
    let $t1 :=
        xdmp:invoke(
            '/feed/lib/actions/subscription-status.xqy'
            ,
...
:)
    ...
    let $req-doc := $req[2]
 	...
 	(: if it is an html doc
 	      remove the subscription document
 	      look for feeds to subscribe to
 	      store the html document
 	   if it is a feed doc
 	      continue processing the feed request
 	:)
    return
   		if ($req-doc/html)
   		then
   			(: delete the subscription docs :)
   			let $t1 := 
   		    	xdmp:invoke(
            		'actions/document-delete.xqy',
            		(xs:QName("doc"),$sub-uri
             		)
        		)
   			
   			let $l12:= xdmp:log(fn:concat("DOC:", fn:node-name($req-doc/html) ))
   			let $link := $req-doc/html/head//link[@rel='alternate'][@type='application/atom+xml' or @type='application/rss+xml']

 			(: store the html page :)
			let $t1-html :=
        		xdmp:invoke(
            		'actions/html-insert.xqy',
            		(xs:QName("html-uri"),$sub-uri,
             		xs:QName("html-doc"),$req-doc,
             		xs:QName("html-col"),$COL-HTML)
        	)  

			for $l in $link/@href
 				let $t1 := xdmp:log(fn:concat("LINK:", $l))
 				return subscribe($l)
    		...
    		return    
        		(process-desc-doc($req-doc, $feed-reference, $feed-type, $time, $versioning, $sub-id, $desc-id),
         		 ...
         		 process-sub($feed-reference-pref, $sub-id, $time) (:SECOND:)
        	    ) 
...
define function process-item-doc($req-doc as item(), 
    ...
    let $t2-item-insert :=
        for $i in $req-items
            ...
            let $description := item-description($i)
...
(: check to see if we have this URL, if so, skip it :)            
let $html :=
for $href in $description//a/@href
return feed:subscribe($href)            
...
define function process-sub ($feed-reference-pref as xs:string,
    ...
    (
        xdmp:invoke(
            '/feed/lib/actions/subscription-reference-preferred.xqy'
            ,
            (xs:QName("sub-id"),$sub-id,
    ...
    (: update the subscription last-requested attribute :)
        xdmp:invoke(
            '/feed/lib/actions/subscription-last-requested.xqy'
            ,
            (xs:QName("sub-id"),$sub-id,
...
define function item-description($f as node()) as item()* {
	 let $desc :=   
		...
		return
			entity:enrich(
			<node xmlns:feed="http://marklogic.com/xdmp/feed" xmlns:e="http://marklogic.com/entity">
				{
				   try {
                       xdmp:unquote($desc, xs:string(""),(xs:string("repair-full"),xs:string("format-xml")) ) 
                   } catch ($e) {
                       try {
                           xdmp:tidy($desc,
                               <options xmlns="xdmp:tidy">
